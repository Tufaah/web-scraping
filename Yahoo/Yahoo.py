...
def yahoo(search :str, page :int):
    ...
    for y in range(len(websites)): data.append(str(titles[y]).replace(str(websites[y]),''))

    for x in range(len(links)): done.append({'link':str(links[x]), 'title':str(data[x]), 'website':str(websites[x]).split()[0], 'description':str(descriptions[x]), })
    print({'results':0, 'data':f'Cant find results for: {search}'} if not done else {'results':len(links), 'data':done})
